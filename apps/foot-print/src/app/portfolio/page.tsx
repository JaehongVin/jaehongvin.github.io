    ...
    projects: [
      {
        title: '팀 개발 기반·채용',
        summary:
          '5개 서비스의 공통 품질 기준을 운영하고, 프론트엔드 채용 면접과 신규 입사자 온보딩 담당',
        details: [
          'pnpm/Turborepo 모노레포의 5개 서비스에 TypeScript strict·ESLint max-warnings 0을 적용하고, 품질 검사 미통과 시 병합 차단',
          '프론트엔드 채용 면접을 맡아 팀 충원에 참여하고 신규 입사자 온보딩 담당',
        ],
      },
      {
      ...
      {
        title: '3개 서비스의 공통 UI·테마·접근성 개선',
        summary:
          '웰로·웰로 비즈·고향사랑기부제가 공유하는 React 디자인 시스템의 컴포넌트·테마·접근성 개선',
        details: [
          '디자이너와 협업해 공통 컴포넌트를 추가·개선하고 3개 서비스에서 재사용',
          '디자인 토큰으로 서비스별 테마를 관리해 공통 UI 구조와 서비스별 스타일을 함께 지원',
          'WCAG 표준에 맞춰 공통 컴포넌트의 접근성을 개선해 사용하는 서비스가 개선 사항을 공유',
        ],
      },
    ],
    problemSolving: [
      {
        title: 'HTTP 메서드·엔드포인트 기반 API 타입 추론 고도화',
        problem:
          'OpenAPI Codegen으로 개별 API 호출 함수를 생성하던 구조에서, HTTP 메서드·엔드포인트 기반 타입 추론을 TanStack Query의 useQuery·useMutation 사용 지점까지 연결할 필요가 있었음.',
        approach:
          'Hey API를 활용해 OpenAPI 스펙에서 타입과 API 요청 함수를 생성하고, HTTP 메서드·엔드포인트 조합에 따라 요청·응답 타입이 추론되도록 API 요청 계층을 개선. 이를 useQuery·useMutation과 연동해 유효한 메서드·엔드포인트, 조회·변경 요청의 입력값과 결과 타입이 사용 지점에서도 추론되도록 구성.',
        result:
          '데이터 조회와 변경 요청부터 useQuery·useMutation의 결과 사용까지 타입 추론을 연결. 생성된 타입을 기준으로 API 요청과 응답 사용 코드의 타입 오류를 개발 단계에서 확인할 수 있도록 개선.',
        note: null,
      },
      {
        title: 'AI 코딩 에이전트의 오류 수정·재검증 자동화',
        problem:
        ...
        result:
          '코드 생성 후 타입·빌드 오류의 수정과 재검증까지 에이전트 실행 루프에 포함. 반복 오류는 공통 규칙에 반영해 이후 작업에도 적용.',
        note: null,
      },
      {
    ...
    <section className="mb-40">
      ...
      <p className="mt-12 text-px-14 leading-px-22 text-gray-600 tb:text-px-15">
        <strong className="font-700 text-gray-900">
          반복되는 문제를 팀이 다시 풀지 않도록, 코드와 개발 방식을 함께 바꾸는
          프론트엔드 엔지니어입니다.
        </strong>{' '}
        5개 서비스의 코드 품질 기준을 통일하고, AI로 에러 분석부터 티켓 생성까지
        자동화했습니다. B2C·B2B·커머스 제품 개발과 성능 개선을 맡으며, FE 채용
        면접과 신규 입사자 온보딩에도 참여했습니다.
      </p>
    </section>
