    ...
    themes: [
      {
        ...
        highlights: [
          {
            title: '5개 서비스의 코드 품질 기준 통일',
            body: 'pnpm/Turborepo 모노레포를 운영하며 TypeScript strict와 ESLint max-warnings 0을 공통 기준으로 적용. 품질 검사를 통과하지 못한 코드는 병합되지 않도록 강제해 5개 서비스에 동일한 품질 기준을 적용.',
          },
          {
            title: 'AI 코딩 에이전트의 오류 수정·재검증 자동화',
            body: '모노레포의 에이전트 규칙을 루트·패키지별로 나눠 수정 범위와 프론트엔드 컨벤션을 명시. 타입 검사·빌드를 실행 루프에 연결해 코드 생성 후 발생한 오류의 수정과 재검증까지 에이전트가 수행하도록 자동화. 반복 오류는 공통 규칙에 반영해 이후 작업에도 적용.',
          },
          {
          ...
          },
          {
            ...
            body: 'OpenAPI Codegen으로 개별 API 호출 함수를 생성하던 구조를, Hey API를 활용해 HTTP 메서드와 엔드포인트 조합에 따라 요청·응답 타입이 추론되는 API 요청 계층으로 개선. OpenAPI 스펙 기반 타입·요청 함수 생성을 TanStack Query의 useQuery·useMutation까지 연결해, 사용 지점에서도 유효한 메서드·엔드포인트와 요청·응답 타입이 추론되도록 구성. 데이터 조회와 변경 요청부터 결과 사용까지 타입 추론을 이어가고, 생성된 타입을 기준으로 API 요청과 응답 사용 코드의 타입 오류를 개발 단계에서 확인할 수 있도록 개선.',
          },
          {
            title: '3개 서비스의 공통 UI·테마·접근성 개선',
            body: '웰로·웰로 비즈·고향사랑기부제가 공유하는 React 디자인 시스템의 공통 컴포넌트를 디자이너와 협업해 확장. 디자인 토큰으로 서비스별 테마를 관리하고 WCAG 표준에 맞춰 공통 컴포넌트의 접근성을 개선해, 3개 서비스가 같은 컴포넌트와 개선 사항을 공유하도록 함.',
          },
          {
            title: 'FE 채용 면접·신규 입사자 온보딩',
            body: '프론트엔드 채용 면접을 맡아 팀 충원에 참여하고, 신규 입사자 온보딩을 담당.',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        highlights: [
          ...
          {
            title: 'Flutter·KB Pay 웹뷰 탐색 흐름 개선',
