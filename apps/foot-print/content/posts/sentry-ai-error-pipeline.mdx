...
에러를 인지한 뒤에도 할 일이 남아있다. Sentry 열어서 스택 트레이스 읽고, 원인 파악하고, Jira에 티켓 만들고, 설명 쓰고, 맞는 Epic에 연결하고, Slack에 팀한테 공유하고. 한 건당 10-15분은 잡아먹는다. 도구만 3-4개를 오간다. 반복적이고, 지루하고, 그래서 바쁠 때는 대충 하게 된다.
...
이 파이프라인의 가치는 AI가 사람을 대체하는 데 있지 않다. **에러 대응의 지루한 플로우를 없애서, 사람이 진짜 중요한 일 — 원인 파악, 우선순위 판단, 수정 — 에 집중할 수 있게 하는 것**이다.
